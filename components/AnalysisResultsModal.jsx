...
function createDeviantShareExplanation(result, index) {
  let content = (
    <Container>
      <Row>
        The majority of files in folder [{result.parent}] have the following
        permissions:
      </Row>
      ...
      <Row>While file [{result.file}] has the following permissions:</Row>
      ...
      <Row>
        Permissions that [{result.file}] has that the majority of files in [
        {result.parent}] do not:
      </Row>
      ...
      <Row>
        Permissions that the majority of files in [{result.parent}] have that [
        {result.file}] does not:
      </Row>
...
function createFileFolderExplanation(result, index) {
  let content = (
    <Container>
      <Row>Parent folder [{result.parent}] has the following permissions:</Row>
...
function createSharingChangesExplanation(result, index) {
  ...
  if (result.type === "new") {
    ...
    content = (
      <Container>
        <Row>
          File [{result.file}] had the following permissions in the first
          snapshot:
        </Row>
        ...
        <Row>
          Meanwhile, file [{result.file}] has the following permissions in the
        ...
        <Row>
          Permissions that [{result.file}] has in the new snapshot that it did
          not in the earlier:
        </Row>
        ...
        <Row>
          Permissions that [{result.file}] had in the earlier snapshot that it
          does not in the more recent:
        </Row>
