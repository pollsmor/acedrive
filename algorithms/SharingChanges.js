...
export default async function AnalyzeSharingChanges(
  first_snapshot_id,
  second_snapshot_id,
  path,
  drive
...
function AnalyzeSharingChangesAlgo(
  first_files,
  second_files,
  path,
  drive,
  ...
  let first_all_ids = [];
  for (let first_file of first_all) first_all_ids.push(first_file.id);

  let second_all_ids = [];
  for (let second_file of second_all) second_all_ids.push(second_file.id);
  ...
  for (let shared_id of shared_ids) {
    ...
    if (JSON.stringify(second_perms_str) !== JSON.stringify(first_perms_str)) {
      ...
      let shared_perms = first_perms_str.filter((x) =>
        second_perms_str.includes(x)
      );
      ...
      let deleted_perms = first_perms_str.filter(
        (x) => !shared_perms.includes(x)
      );
...
function addAllFiles(folder, all_files, path, drive) {
  for (let file of folder) {
    if (drive !== "" && file.driveName !== drive) continue;

    // if we are in the path, add this file to the list to be analyzed
    if (file.path.indexOf(path) === 0) all_files.push(file);

    if (file.isFolder) addAllFiles(file.content, all_files, path, drive);
  }
}
