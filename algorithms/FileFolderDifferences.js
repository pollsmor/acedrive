...
export default async function AnalyzeFileFolderDifferences(
  snapshot_id,
  path,
  drive
) {
  ...
  for (let parent_file of files) {
    ...
    if (parent_file.mimeType === "application/vnd.google-apps.folder") {
      ...
      if (parent_file.path.indexOf(path) !== 0) {
        console.log(
          `Skipping analysis on file contents of ${parent_file.path}`
        );
        // Subdirectory might be path though, so continue DFS
        AnalyzeFileFolderDifferencesAlgo(
          parent_file.content,
          path,
          drive,
          result
      ...
      for (let subfile of parent_file.content) {
        ...
        if (parent_perms_string !== subfile_perms_string) {
          ...
          let shared_perms = subfile_permissions.filter((x) =>
            parent_permissions.includes(x)
          );

          // get the perms the subfile has but not the folder
          let extra_perms = subfile_permissions.filter(
          ...
          let missing_perms = parent_permissions.filter(
            (x) => !shared_perms.includes(x)
          );
