...
export function permissionSearch(files, searchTerm, negated, roles) {
  ...
  for (let file of files) {
    ...
    for (let permission of file.permissions) {
      if (roles.includes(permission.role) && permission.email.toLowerCase() === searchTerm.toLowerCase()) {
        perm_found = true;
        break;
      }
    }
...
export function sharedToSearch(files, searchTerm, negated) {
  ...
  for (let file of files) {
    ...
    for (let permission of file.permissions) {
      ...
      if (permission.email.toLowerCase() === searchTerm.toLowerCase()) {
        perm_found;
        break;
      }
    }
...
export function fromUserSearch(files, searchTerm, negated) {
  let results = [];
  for (let file of files) {
    if (file.sharingUser?.emailAddress.toLowerCase() === searchTerm.toLowerCase()) {
      if (!negated) {
        results.push(file);
      }
    } else {
...
export const sharingSearch = (
  ...
  const searchedFiles = files.filter((file) => {
    //TODO: Check if needs to how file is shared using permissions or not 
    ...
    if (Array.isArray(fileDetails)) {
      matchedDetails = lodash.filter(fileDetails, (detail) => {
        ...
        } else if (detail.type.toLowerCase() === searchTerm.toLowerCase()) {
          return detail;
        }
      });
      if (fileDetails.length === 0 && searchTerm === "none") {
        return file;
      }
