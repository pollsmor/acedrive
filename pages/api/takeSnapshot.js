...
export default async function handler(req, res) {
	...
	console.log("Hey From Snap Shot", token);
	try {
		if (token && req.method === "POST") {
			...
			res.json({ message: "Not signed in or not a POST request." });
		}
	} catch (error) {
		console.log(error);
		res.json({ message: "Not signed in or not a POST request." });
	...
	try {
		...
		for (let i = 0; i < all_files.length; i++) {
			...
			if (!file.parents || file.parents.includes(file.driveId)) {
				...
				//check for the user have any permission over the content in the file
				...
				if (file.permissions) {
					...
					file_object.permissions = permissions;
				}
